...
| **Obsidian Web Clipper** | Clip articles and selections straight into `00 - Inbox/` with the vault's own frontmatter and filename prefixes; also the default web-highlight capture the vault has | Browser extension (Chrome/Firefox/Safari incl. iOS/Edge) | **High** — keyless, first-party, schema-exact. Covers the *capture* half of the Article workflow today. Parallel fast-path to Templater Source Capture, not a replacement: no vault read access, so no stub-birthing (Lecture/Series) and no `mx-uid` (Video). Tracked as issues #64–#66; see `research/obsidian-web-clipper.md`. |
...
| **Glasp** | Social web highlighter: public highlight discovery/following layer, with an Obsidian community plugin (`obsidian-glasp-plugin`) that does scheduled, incremental, cursor-tracked sync of web highlights, Kindle highlights, bookmarks, and AI summaries into vault Markdown | Browser extension + Obsidian plugin (API access token) | **Optional companion tool, adopter opt-in only — never a default, never wired into any `Scripts/sourceCapture*.js` module.** Web Clipper's Highlighter stays the default way to get web highlights into this vault (no account, private by default); Glasp is for readers who specifically want the social/discovery layer on top of that. Two caveats to weigh before adopting: **(1)** the free tier is public-by-default with **no private opt-out** — "free account" ≠ "free and private"; keeping highlights private requires Pro ($12.50/mo) or Unlimited ($30/mo). **(2)** the plugin's sync is a **full overwrite on every re-sync** (`vault.modify()`, not a merge) — any hand edit made directly inside an already-imported note is silently wiped on the next scheduled run, so treat Glasp-imported notes as read-only captured material, same as every other Source note. The access token is also stored in plaintext in the plugin's own `data.json`, not Obsidian's `secretStorage`. Glasp-imported notes are a bare importer (no `id`/`type`/`growth`/`status`/`review` frontmatter) and need the same manual reconciliation into `(TEMPLATE) Source Capture` that Snipd/PastReads already require — see the Article Workflow note in WORKFLOWS.md (issue #70). |
...
| **Screenshot-to-LaTeX** | Screenshot → LaTeX conversion | Obsidian plugin (local) | Recommend the community plugin `obsidian-latex-ocr` (or `obsidian-ocrlatex`) in local mode, running pix2tex/Texify against a local `127.0.0.1` server — keyless, ADR 0014 tier 0. Mathpix itself is tier 3 (paid API, no free tier) and not recommended; the adopter can still point either plugin at Mathpix or SimpleTex manually as a higher-accuracy, adopter-supplied-key option, but that is not what ships as the default. |
